...
const oStorage = oMulter.diskStorage({
  destination: (oRequest, oFile, oCallback) => {
    oCallback(null, "public/images/bundles");
  },
  filename: (oRequest, oFile, oCallback) => {
    const sExtension = oFile.mimetype.split("/")[1];
    const sId = oRequest.bundle === undefined ? `bundle-${oRequest.profile._id}-${Date.now()}.${sExtension}` : oRequest.bundle.bundle_thumbnail;
    oCallback(null, sId);
  }
});
...
exports.uploadImage = oUpload.fields([{ name: "bundle_thumbnail" }]);
